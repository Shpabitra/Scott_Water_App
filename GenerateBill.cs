        ...
        private void btnGenerateBill_Click(object sender, EventArgs e)
        {
            using (var db = new ScotWaterContext())
            {
                ...
                var reading = db.Readings
                    .Where(r => r.BusinessID == budinessId &&
                ...
                if (reading == null)
                {
                    MessageBox.Show("No readings found for the selected business and date");
                    return;
                }

                if (price == null)
                {
                    MessageBox.Show("No price found for the selected date");
                    return;
                }

                if (waterLevel == null)
                {
                    MessageBox.Show("No water level found for the selected business and date");
                    return;
                }
                MessageBox.Show(
                "Price Effective Date:" + price.EffectiveDate.ToShortDateString() +
                        ...
                        "\nNoDrought Tier 1" + price.NoDroughtTier1Rate +
                        "\nNoDrought Tier 2" + price.NoDroughtTier2Rate +
                        "\nNoDrought Tier 3" + price.NoDroughtTier3Rate
                        );
